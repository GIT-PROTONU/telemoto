...
move_group /get_planning_scene service, and verifies the floor-ACM safety
contract:
  A. before the ACM is fetched: 5 walls published, NO floor, NO ACM
     (a floor without its ACM would pin Servo's collision monitor at contact)
  B. once the service answers: floor added, ACM extended (floor allowed
     against base/shoulder links only), original entries preserved
...
# A ── no ACM service yet: the floor must be withheld ─────────────────────────
time.sleep(3.0)                                   # a few keepalives
ids = {o.id for s in scenes for o in s.world.collision_objects}
check("A 5 walls, floor withheld pre-ACM", ids == WALLS5, str(sorted(ids)))
...
NAMES = ["base_link", "base_link_inertia", "shoulder_link", "wrist_1_link"]
...
if got is not None:
    check("B all 6 cage walls published",
          {o.id for o in got.world.collision_objects} == WALLS5 | {"wall_floor"})
    acm = got.allowed_collision_matrix
    def allowed(a, b):
        return bool(acm.entry_values[acm.entry_names.index(a)]
                    .enabled[acm.entry_names.index(b)])
    check("B ACM carries the floor entry", "wall_floor" in acm.entry_names,
          str(list(acm.entry_names)))
...
check("C drag saved to pose file",
      ...
      and len(saved) == 6,
      f"wall_top z={saved['wall_top']['position'][2]}, {len(saved)} walls")
